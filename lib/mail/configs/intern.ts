// 무료 공고 제안메일(intern) 생성기 config.
// 기본 제목·본문·직무분야 옵션은 프로토타입(intern.{html,js})에서 verbatim 이식.

import { dateParts, computeStatPeriod } from "../engine/format";
import type { FieldOption, GeneratorConfig } from "./types";

// 직무 분야 셀렉트(8): value = 선정이유 문장, label = 분야명(=직무명).
const JOB_FIELD_OPTIONS: FieldOption[] = [
  {
    value: "마케팅 실무 경험을 쌓고자 하는 취업준비생의 관심도가 높은 직무로",
    label: "마케팅",
  },
  {
    value: "콘텐츠 제작과 채널 운영 경험을 원하는 이용자에게 적합한 공고로",
    label: "콘텐츠",
  },
  {
    value: "디자인 실무와 포트폴리오 경험을 함께 쌓을 수 있는 공고로",
    label: "디자인",
  },
  {
    value: "실제 개발 환경과 프로젝트 경험을 원하는 이용자에게 유용한 공고로",
    label: "개발",
  },
  {
    value: "서비스 기획과 운영 실무를 경험하려는 취업준비생에게 적합한 공고로",
    label: "기획·운영",
  },
  {
    value: "조직 운영과 채용·인사 실무에 관심 있는 이용자에게 유용한 공고로",
    label: "HR·경영지원",
  },
  {
    value: "고객 접점과 사업개발 경험을 원하는 이용자에게 적합한 공고로",
    label: "영업·사업개발",
  },
  {
    value: "인턴십을 통해 실제 직무 경험을 쌓고자 하는 이용자에게 유용한 공고로",
    label: "기타",
  },
];
...
const FACTORY_DEFAULT_BODY = `<div>안녕하세요, {{회사명}} 채용 담당자님.</div>
<div>취업정보 플랫폼 렛츠커리어입니다.</div>
<div><br></div>
<div>현재 모집 중인 인턴 채용공고를 검토한 결과,</div>
<div>귀사의 「{{공고명}}」을 렛츠커리어 인스타그램 <b>'오늘의공고'</b>의 1회 소개 대상으로 선정해 연락드립니다.</div>
<div><br></div>
<div>'오늘의공고'는 취업준비생이 놓치기 쉬운 채용 기회를 선별해,</div>
<div>모집 직무와 주요 지원조건, 일정을 한눈에 소개하는 채용공고 큐레이션 콘텐츠입니다.</div>
<div><br></div>
<div>귀사의 이번 공고 역시 {{선정이유}} 판단했습니다.</div>
<div><br></div>
<div>-----</div>
<div><br></div>
...
<div>-----</div>
<div><br></div>
<div>이번 소개 대상으로 선정된 공고는 기업별 1건, 1회에 한해 <b>별도의 콘텐츠 제작비나 게시 비용 없이</b> 소개합니다.</div>
<div><br></div>
<div>게시물에는 공개된 채용공고를 기준으로 다음 내용을 정리해 담을 예정입니다.</div>
<div><br></div>
<div>- 회사명 및 모집 직무</div>
<div>- 주요 지원조건</div>
<div>- 모집 마감일</div>
...
<div><br></div>
<div>이번 소개가 귀사의 좋은 채용 기회와 적합한 지원자를 연결하는 데 도움이 되길 바랍니다.</div>
<div><br></div>
<div>감사합니다.</div>
<div><br></div>
...
export const INTERN_IMAGE_PLACEHOLDER =
  "[렛츠커리어 JOB 인스타그램 최근 2주 성과 이미지 삽입]";

// 첫 옵션을 기본 선택값으로 둔다(프로토타입 select 기본 동작과 일치).
export const INTERN_DEFAULT_FIELD_VALUES: Record<string, string> = {
  jobFieldReason: JOB_FIELD_OPTIONS[0].value,
};

export const internConfig: GeneratorConfig = {
  key: "intern",
  label: "무료공고",
  storagePrefix: "internMailTool",
  factorySubject: FACTORY_DEFAULT_SUBJECT,
  factoryBody: FACTORY_DEFAULT_BODY,
  josaVars: ["회사명", "직무명", "공고명"],
  rawHtmlKeys: [],
  fields: [
    { id: "companyName", label: "회사명", type: "text", placeholder: "예: 렛츠커리어" },
    {
      id: "jobFieldReason",
      label: "직무 분야",
      type: "select",
    ...
    },
    {
      ...
      label: "공고명",
      type: "text",
      placeholder: "예: 2026년 하반기 마케팅 인턴 채용",
    },
